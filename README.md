...
Under the hold, PyDepta implemented Yanhong Zhai and Bing Liu's work on [Web Data Extraction Based on Partial Tree Alignment](http://dl.acm.org/citation.cfm?id=1060761)
to extract data without example data (so called unsupervised learning).
The basic idea is of this algorithm is to extract the data region with tree match algorithm (see Bing Lius' previous work on [MDR](http://dl.acm.org/citation.cfm?id=956826))
